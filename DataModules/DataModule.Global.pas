...
uses
  ...
  FireDAC.Phys,
  UnitPrincipal, FireDAC.Stan.ExprFuncs;
...
begin
  ...
  Connection.ConnectionDefName := 'MSSQL_Walquiria';  // exatamente como no FireDAC Explorer
  with Connection.Params do
  begin
    Clear;
    Add('DriverID=ODBC');
    Add('ODBCDataSource=Walquiria'); // seu DSN testado
    Add('User_Name=sa');
    Add('Password=sa');
    // Se o DSN estiver com criptografia obrigatória e sem CA pública:
    // Add('Encrypt=yes');
    // Add('TrustServerCertificate=yes');
    // (Opcional) se quiser forçar DB padrão (caso o DSN não defina):
    // Add('Database=Northwind');
    Add('ODBCVersion=3.0');
  end;
end;
